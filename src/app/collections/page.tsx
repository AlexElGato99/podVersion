        ...
        <div className="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-3">
          {collections.map((col) => (
            <Link
              key={col.name}
              href={col.href}
              className="group relative overflow-hidden rounded-3xl border border-zinc-800 bg-zinc-900 transition-all hover:border-zinc-700 hover:-translate-y-1 hover:shadow-2xl"
            >
              <div
                className={`h-48 bg-gradient-to-br ${col.gradient} opacity-80 group-hover:opacity-100 transition-opacity`}
              />
              <div className="p-6">
                <h2 className="text-lg font-bold text-zinc-100 group-hover:text-brand-300 transition-colors">
                  {col.name}
                </h2>
                ...
                <div className="mt-4 flex items-center justify-between">
                  ...
                  <span className="flex items-center gap-1 text-sm font-medium text-brand-400 group-hover:gap-2 transition-all">
                    Shop now
